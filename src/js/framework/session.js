...
var Session = core.Class.extend(mixins.EventDispatcherMixin, {
    ...
    init: function(parent, origin, options) {
        ...
        options = options || {};
        this.module_list = (options.modules && options.modules.slice()) || (window.ecore._modules && window.ecore._modules.slice()) || [];
        this.server = null;
    ...
    user_has_group: function(group) {
        ...
        if (!def) {
            var Model = window.ecore.web.Model;
    ...
    load_currencies: function() {
        this.currencies = {};
        var self = this;
        return new ecore.web.Model("res.currency").query(["symbol", "position", "decimal_places"]).all()
                .then(function(value) {
    ...
    on_modules_loaded: function() {
        ...
        for(var j=0; j<this.module_list.length; j++) {
            ...
            ecore[mod] = {};
            // init module mod
            var fct = ecore._ecore[mod];
            if(typeof(fct) === "function") {
                ecore._ecore[mod] = {};
                for (var k in fct) {
                    ecore._ecore[mod][k] = fct[k];
                }
                fct(ecore, ecore._ecore[mod]);
            }
        ...
        return self.check_session_id().then(function() {
            ...
            if (self.origin_server) {
                ...
                if (self.override_session) {
                    options.headers["X-Ecore-Session-Id"] = self.session_id || '';
                }
...
ecore.define('web.config', function (require) {
"use strict";
