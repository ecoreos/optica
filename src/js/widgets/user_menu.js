ecore.define('web.UserMenu', function (require) {
"use strict";
...
var UserMenu = Widget.extend({
    ...
    on_menu_documentation: function () {
        window.open('https://www.ecore.cloud/documentation/user', '_blank');
    },
    on_menu_support: function () {
        window.location.href = 'mailto:help@ecore.cloud';
    },
    on_menu_about: function () {
        var self = this;
        if (ecore.db_info) {
            menu_help_about(ecore.db_info);
        } else {
    ...
    on_menu_account: function() {
        ...
        P.call('get_param', ['database.uuid']).then(function(dbuuid) {
            ...
            window.location.href = 'https://accounts.ecore.cloud/oauth2/auth?'+$.param(params);
        }).fail(function(result, ev){
            ev.preventDefault();
            window.location.href = 'https://accounts.ecore.cloud/account';
        });
    },
